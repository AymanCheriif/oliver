...
import type { Locale, TranslationKey, I18nAPI, Section } from "./types";
...
export class EnterpriseI18nSystem implements I18nAPI {
  ...
  constructor() {
    ...
    this.cacheManager = TranslationCacheManager.getInstance(
      I18N_CONFIG.cacheConfig
    );
  ...
  private initializeVueI18n(): void {
    ...
    console.log(
      `[I18N] Vue i18n initialized with locale: ${this.vueI18n.global.locale.value}`
    );
  }
  ...
  private createMissingHandler() {
    return (locale: Locale, key: TranslationKey) => {
      try {
        // Load locale if not loaded (avoid infinite loop)
        if (!this.stateManager.isLocaleLoaded(locale)) {
          // Fire-and-forget load; missing handler must be sync for vue-i18n
        ...
        // Key-level loading skipped in sync missing handler (would require async)

        // Try fallback locale if current locale is not fallback
        if (locale !== I18N_CONFIG.fallbackLocale) {
          const fallbackMessages = this.vueI18n.global.getLocaleMessage(
  ...
  public async setLocale(locale: Locale): Promise<boolean> {
    try {
      ...
      this.stateManager.completeLoading(
        { locale, timestamp: Date.now() } as any,
        {
          success: false,
  ...
  public getTranslation(key: TranslationKey, locale?: Locale): string {
    ...
    try {
      ...
      console.warn(
        `[I18N] Failed to get translation for '${key}' in '${targetLocale}':`,
        error
      );
  ...
  public async preloadLocale(locale: Locale, section?: Section): Promise<void> {
    try {
      // If locale already fully loaded, skip
      if (this.stateManager.isLocaleLoaded(locale)) {
        if (process.env.NODE_ENV !== "production") {
          console.log(
            `[I18N] (cache) Locale '${locale}' already loaded - skipping preload${
              section ? ` (requested section '${section}')` : ""
            }`
          );
        }
        return;
      }
      // Always load full locale (section slicing happens in loader but we benefit from full cache)
      if (process.env.NODE_ENV !== "production") {
        console.log(
          `[I18N] Preloading full locale '${locale}'${
            section ? ` (requested section '${section}')` : ""
          }`
        );
      }
      ...
      if (process.env.NODE_ENV !== "production") {
        console.log(`[I18N] Preload complete for '${locale}'`);
      }
    } catch (error) {
      console.error(
        `[I18N] Failed to preload locale '${locale}'${
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      if (initialLocale !== I18N_CONFIG.fallbackLocale) {
        try {
          const fallbackTranslations = await this.loadingStrategy.load(
            I18N_CONFIG.fallbackLocale
          );
          this.vueI18n.global.setLocaleMessage(
            I18N_CONFIG.fallbackLocale,
...
export const preloadLocale = (locale: Locale) =>
  enterpriseI18n.preloadLocale(locale);
...
export type { Locale, TranslationKey, I18nAPI };
